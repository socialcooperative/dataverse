...
ENV NGINX_ROOT="/home/app/public"
# display PHP errors
ENV ERRORS=0
...
COPY ./inc/server_nginx.conf.tmpl /etc/nginx/sites-available/default.conf
...
# Supervisor Config
COPY ./inc/supervisord.conf /etc/supervisord.conf
...
RUN cd ${APP_ROOT} && su app -c "APP_ENV=prod composer install --no-dev --no-interaction -o"

COPY ./docker_extra.sh /
RUN chmod 755 /docker_extra.sh

# Expose Ports
EXPOSE 80
CMD ["/bin/bash", "/docker_extra.sh"]
# CMD echo "prepare script starting" && /bin/bash /docker_extra.sh && echo "tailing..." && : >> /docker_extra.log && tail -f /docker_extra.log
